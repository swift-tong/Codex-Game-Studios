...
Spawn `technical-director` via spawn_agent using gate **TD-CHANGE-IMPACT** (`.agents/docs/director-gates.md`).
...
- **CONCERNS** → surface the specific ADRs or recommendations flagged in plain text with options: `Revise the impact assessment` / `Accept with noted concerns` / `Discuss further`
