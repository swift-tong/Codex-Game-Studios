...
Ask in plain text with two short prompts:
...
Present the art-director's draft to the user and ask in plain text:
...
Collect both. If they conflict, surface the conflict explicitly with both positions. Do not silently resolve - ask the user to decide in plain text.
...
After all sections are complete (or the scoped set from Phase 1 is complete), spawn `creative-director` via spawn_agent using gate **AD-ART-BIBLE** (`.agents/docs/director-gates.md`).
...
Ask in plain text about next steps. Only include options that are genuinely next based on the state check above:
