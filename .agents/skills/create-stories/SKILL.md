...
After decomposing all stories (Step 4 complete) but before presenting them for write approval, spawn `qa-lead` via spawn_agent using gate **QL-STORY-READY** (`.agents/docs/director-gates.md`).
...
Use a plain-text closing question with context-aware next steps:
