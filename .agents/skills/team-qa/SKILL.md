...
Present the qa-lead's full strategy to the user, then ask in plain text:
...
Ask in plain text per story group (batched 3-4 at a time):
...
Batch stories into groups of 3-4 and ask in plain text for each:
...
After each FAIL result, ask in plain text for the failure description, then spawn `qa-tester` via spawn_agent to write a formal bug report in `production/qa/bugs/`.
