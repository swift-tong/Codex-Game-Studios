...
user-invocable: true
allowed-tools: Read, Glob, Grep, Write, Edit, TodoWrite, spawn_agent
---
...
Then ask for feedback in plain text:
...
Ask in plain text: "Does this dependency ordering look right? Any
...
**After dependency mapping is approved, spawn `technical-director` via spawn_agent using gate TD-SYSTEM-BOUNDARY (`.agents/docs/director-gates.md`) before proceeding to priority assignment.**
...
Ask in plain text: "Do these priority assignments match your vision?
...
**After priorities are approved, spawn `producer` via spawn_agent using gate PR-SCOPE (`.agents/docs/director-gates.md`) before writing the index.**
...
**After the systems index is written, spawn `creative-director` via spawn_agent using gate CD-SYSTEMS (`.agents/docs/director-gates.md`).**
...
Ask in plain text: "Start designing [system-name] now, pick a different
system, or stop here?"
...
After `/design-system` completes, ask in plain text:
...
After the systems index is created (or after designing some systems), present next actions in plain text:
...
2. **Ask clearly** at every decision point (Explain -> Capture pattern):
