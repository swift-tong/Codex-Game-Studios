...
user-invocable: true
allowed-tools: Read, Glob, Grep, Bash, Edit, spawn_agent
---
...
3. If multiple in-progress stories are found, ask in plain text:
...
### Manual verification with confirmation (ask in plain text)
...
Batch up to 4 manual verification questions into a single plain-text round:
...
- **Manual confirmation**: if the criterion was verified through explicit plain-text confirmation
...
After completing the deviation checks in Phase 4, spawn `qa-lead` via spawn_agent using gate **QL-TEST-COVERAGE** (`.agents/docs/director-gates.md`).
...
Spawn `lead-programmer` via spawn_agent using gate **LP-CODE-REVIEW** (`.agents/docs/director-gates.md`).

Pass: implementation file paths, story file path, relevant GDD section, governing ADR.

Present the verdict to the user. If CONCERNS, surface them via plain-text options:
...
- Use plain-text questions for the code review prompt and for batching manual
