...
2. If a version is found: report "No version argument provided - inferred [version] from milestone data. Proceeding." Then confirm in plain text.
3. If no version is discoverable: ask in plain text what version number should be released and wait for user input before proceeding. Do not default to a hardcoded version string.

When this skill is invoked, orchestrate the release team through a structured pipeline.

**Decision Points:** At each phase transition, ask in plain text
