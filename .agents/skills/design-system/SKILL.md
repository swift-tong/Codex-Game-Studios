...
user-invocable: true
allowed-tools: Read, Glob, Grep, Write, Edit, spawn_agent, TodoWrite
---
...
2. If it exists: read it, find the highest-priority system with status "Not Started" or equivalent, and ask in plain text whether to design that system, pick a different one, or stop here:
...
2. **Questions**: Ask clarifying questions specific to this section. Use
   plain-text options for constrained questions, and conversational text for
...
3. **Options**: Where the section involves design choices (not just documentation),
   ...
   then ask the user to make the decision in plain text.
...
6. **Approval**: Immediately after the draft — in the SAME response — use
   plain-text approval. **Never skip this step.**
...
**Framing questions (ask BEFORE drafting)**: Ask these in plain text before drafting:
...
**Framing question (ask BEFORE drafting)**: Ask in plain text:
...
- Surface any disagreements between agents to the user in plain text
...
- Present the specialists' proposals to the user for review in plain text
...
Before finalizing the GDD, spawn `creative-director` via spawn_agent using gate **CD-GDD-ALIGN** (`.agents/docs/director-gates.md`).
...
- The main session presents the agent's output to the user in plain text
- The user decides; the main session writes to file
