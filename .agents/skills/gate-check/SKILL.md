...
Before generating the final verdict, spawn all four directors as **parallel subagents** via spawn_agent using the parallel gate protocol from `.agents/docs/director-gates.md`. Issue all four spawn_agent calls simultaneously — do not wait for one before starting the next.
...
After the verdict is presented and any stage.txt update is complete, close with a short plain-text next-step prompt.
...
- **No accessibility requirements doc?** Ask in plain text whether the user wants it created now:
  ...
  - If "Create it now": ask in plain text which accessibility tier fits best:
