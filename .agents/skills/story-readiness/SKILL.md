...
user-invocable: true
allowed-tools: Read, Glob, Grep, spawn_agent
model: haiku
---
...
**Scope:** `$ARGUMENTS[0]` (blank = ask the user in plain text)
...
If no argument is given, ask in plain text:
...
Spawn `qa-lead` via spawn_agent using gate **QL-STORY-READY** (`.agents/docs/director-gates.md`).
...
- **GAPS [list]** -> surface the specific gaps to the user in plain text:
